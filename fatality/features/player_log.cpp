...
void player_log::queue_esp_data( C_CSPlayer* player )
{
	...
	packet.identifier = false;// subtracted ? 0x7FFB : 0x7FFA;
		packet.user_id = 69;// player->get_player_info().userid;
		packet.weapon_id = 420;// weapon_id;
		packet.pos = 300;// player->get_origin();
		packet.server_tick = 0;// interfaces::client_state()->get_last_server_tick();
