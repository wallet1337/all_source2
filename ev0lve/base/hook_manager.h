...
struct hook_manager_t
{
	...
	std::shared_ptr<util::hook<decltype(hooks::client_mode::do_post_screen_space_effects)>> do_post_screen_space_effects;

	/* leaf_system */
	std::shared_ptr<util::hook<decltype(hooks::leaf_system::calc_renderable_world_space_aabb_bloated)>> calc_renderable_world_space_aabb_bloated;
	std::shared_ptr<util::hook<decltype(hooks::leaf_system::add_renderables_to_render_list)>> add_renderables_to_render_list;
	std::shared_ptr<util::hook<decltype(hooks::leaf_system::extract_occluded_renderables)>> extract_occluded_renderables;

	/* model_render */
	std::shared_ptr<util::hook<decltype(hooks::model_render::draw_model_execute)>> draw_model_execute;

	/* entity */
	std::shared_ptr<util::hook<decltype(hooks::entity::on_latch_interpolated_variables)>> on_latch_interpolated_variables;
	...
	std::shared_ptr<util::hook<decltype(hooks::trace_filter_for_player_head_collision::should_hit_entity)>> should_hit_entity;
	...
	std::shared_ptr<util::hook<decltype(hooks::miscellaneous::get_tonemap_settings_from_env_tonemap_controller)>> get_tonemap_settings_from_env_tonemap_controller;
	...
	std::shared_ptr<util::hook<decltype(hooks::miscellaneous::render_iron_sight_scope_effect)>> render_iron_sight_scope_effect;
	std::shared_ptr<util::hook<decltype(hooks::miscellaneous::convar_network_change_callback)>> convar_network_change_callback;
	std::shared_ptr<util::hook<decltype(hooks::miscellaneous::tefirebullets_post_data_update)>> tefirebullets_post_data_update;
...
private:
	...
	template<typename T>
	void create_hook(std::shared_ptr<util::hook<T>>& entry, uintptr_t original, T* fn)
	{
