...
protected:
	struct event_element
	{
		...
		event_element(int col, std::string txt): dec_text(std::move(txt))
		{
			color = col;
		}
	};
...
public:
	std::map<int, sdk::color> console_colors = {
		{ 0x01, sdk::color(255, 255, 255) },
		{ 0x02, sdk::color(255, 0, 0) },
		{ 0x03, sdk::color(185, 130, 240) },
		{ 0x04, sdk::color(0, 255, 0) },
		{ 0x05, sdk::color(190, 255, 140) },
		{ 0x06, sdk::color(165, 255, 80) },
		{ 0x07, sdk::color(255, 60, 60) },
		{ 0x08, sdk::color(200, 200, 200) },
		{ 0x09, sdk::color(225, 210, 125) },
		{ 0x0A, sdk::color(175, 195, 215) },
		{ 0x0B, sdk::color(95, 150, 216) },
		{ 0x0C, sdk::color(75, 105, 255) },
		{ 0x0D, sdk::color(175, 195, 215) },
		{ 0x0E, sdk::color(210, 40, 230) },
		{ 0x0F, sdk::color(235, 75, 75) },
	...
	void add(int color, std::pair<std::string, char> text)
	{
		log.emplace_back(event_element(color, std::move(text)));
	}

	void add(int color, std::string text)
	{
		log.emplace_back(event_element(color, std::move(text)));
	}
...
private:
	template<typename ...A>
	void output_console(int clr, const char* format, A ...args);
	template<typename ...A>
	void output_console(const char* format, A ...args);

	template<typename ...A>
	void output_chat(int clr, const char* format, A ...args);
	template<typename ...A>
	void output_chat(const char* format, A ...args);
...
template<typename ...A>
inline void event_log::output_console(int clr, const char* format, A ...args)
{
	char out[1024];
...
template<typename ...A>
inline void event_log::output_console(const char* format, A ...args)
{
	char out[1024];
	sprintf_s(out, 1024, format, args...);
...
template<typename ...A>
inline void event_log::output_chat(int clr, const char* format, A ...args)
{
	char out[1024];
	sprintf_s(out, 1024, format, args...);
...
template<typename ...A>
inline void event_log::output_chat(const char* format, A ...args)
{
	char out[1024];
	sprintf_s(out, 1024, format, args...);
