...
public:
	...
	virtual void draw_player(sdk::cs_player_t* const player, const sdk::mat3x4* mat, sdk::color clr = sdk::color(0, 255, 0, 130),
		float duration = game->globals->interval_per_tick, std::optional<int32_t> only_hitbox = std::nullopt) const;
...
template<typename ...A>
inline void print_to_console(const char* format, A ...args)
{
	char out[0x1000];
...
template<typename ...A>
inline void print_to_console(sdk::color clr, const char* format, A ...args)
{
	char out[0x1000];
	sprintf_s(out, sizeof(out), format, args...);
	auto cl = clr;
