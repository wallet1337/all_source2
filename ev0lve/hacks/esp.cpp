...
namespace hacks
{
	void esp::draw(const gui_legacy::draw_adapter& draw)
	{
		...
		game->client_entity_list->for_each([&](entity_t* ent)
		{
			if (!ent->is_entity())
				return;

			if (ent->is_base_combat_weapon()
				&& game->client_entity_list->get_client_entity_from_handle(reinterpret_cast<weapon_t*>(ent)->get_owner_entity()))
				return;

			auto& entry = closest_entity[size_t(ent->get_class_id())];
			const auto dist = (game->view_render->get_view_setup()->origin -
				ent->get_abs_origin()).length();

			if (!entry.first || dist <= entry.second)
				entry = { ent, dist };
		});

		game->client_entity_list->for_each_z([&](entity_t* const entity)
		{
	...
	{
		auto observer_target = reinterpret_cast<cs_player_t* const>(game->client_entity_list->get_client_entity(game->engine_client->get_local_player()));

		if (observer_target && (player->get_observer_mode() == 4 || player->get_observer_mode() == 5))
			observer_target = reinterpret_cast<cs_player_t* const>(game->client_entity_list->get_client_entity_from_handle(observer_target->get_observer_target()));

		if (player == observer_target || player->index() == game->engine_client->get_local_player())
			return;

		if (!player->is_alive() && !player->is_dormant() && (player->get_observer_mode() == 4 || player->get_observer_mode() == 5))
		{
			observer_target = reinterpret_cast<cs_player_t* const>(game->client_entity_list->get_client_entity_from_handle(player->get_observer_target()));
			if (observer_target && observer_target->is_player() && observer_target->is_dormant())
			{
				...
				vis.last_update = game->client_state->last_server_tick;
				vis.alpha = .3f;
			}
		}
		...
		if (!player->is_dormant())
		{
			...
			player_entry.visuals.last_update = game->client_state->last_server_tick;
		}
		...
		linear_fade(alpha, player_entry.visuals.last_update < game->client_state->last_server_tick - TIME_TO_TICKS(5.f)
			? 0.f : .3f, .7f, 2.f, !player->is_dormant());

		auto& cf = player->is_enemy() ? cfg.player_esp.enemy : cfg.player_esp.team;

		if (!cf.get().first_set_bit())
			return;

		const auto visible = is_visible(player->get_eye_position());
		const auto& clr = (cfg.player_esp.vis_check.get() && visible) ?
		...
		const auto health_bar = std::make_pair(vec2(box->first.x - 6, box->first.y - 1), vec2(box->first.x - 2, box->second.y + 1));
		...
		const auto armor_bar = std::make_pair(vec2(box->second.x + 2, box->first.y - 1), vec2(box->second.x + 6, box->second.y + 1));
		...
		if (!box.has_value())
			return draw_player_offscreen(draw, player, clr);
		...
		if (player->is_enemy() ? cfg.player_esp.enemy_skeleton.get() : cfg.player_esp.team_skeleton.get())
			draw_bones(draw, player, saturate(player->is_enemy() ? cfg.player_esp.enemy_skeleton_color.get() : cfg.player_esp.team_skeleton_color.get(), fractional));

		const auto ammo_bar = std::make_pair(vec2(box->first.x - 1, box->second.y + 2), vec2(box->second.x + 1, box->second.y + 6));
		
		auto padding = vec3(4, 0, 4);

		if (cf->test(cfg_t::esp_text))
			draw.text({ box->first.x + (box->second.x - box->first.x) / 2.f, box->first.y + 1 }, saturate(color::white(), fractional).alpha(clr_a),
					  get_player_name(player).c_str(), FNV1A("segoeuib13"), gui_legacy::horizontal_alignment::center, gui_legacy::vertical_alignment::bottom);
		
		if (cf->test(cfg_t::esp_box))
			draw_box(draw, box.value(), saturate(clr, fractional));

		if (cf->test(cfg_t::esp_health_bar))
		{
			padding.x *= 2;
			draw_bar(draw, health_bar, interpolate(color::health_end(), color::health_start(), current_health, fractional).alpha(clr_a), current_health, player_entry.visuals.previous_health,
				cf.get().test(cfg_t::esp_health_text) && current_health <= .99f);
		}
		...
		if (cf->test(cfg_t::esp_distance))
		{
			...
			if (dist > 0)
			{
				draw.text({ box->first.x - padding.x - 1, box->first.y }, color::white().alpha(clr_a), XOR_FORMAT_UTF8("%d YD", dist),
						FNV1A("small8"), gui_legacy::horizontal_alignment::right);
				pad += 9;
			}
		}
		...
		{
			draw.icon({ box->first.x - padding.x - 7, box->first.y + pad }, 12.f, saturate(
					(player_entry.is_exploiting() ? color::attention() : color::white()), fractional).alpha(clr_a), XOR_STR("icons/ui/warning"));
			pad += 12;
		}
		...
		{
			padding.z *= 2;
			draw_bar(draw, armor_bar, saturate(color::armor(), fractional).alpha(clr_a), current_armor, player_entry.visuals.previous_armor);
		}

		if (cf->test(cfg_t::esp_flags) && player_entry.is_visually_fakeducking())
		{
			draw.text({box->second.x + padding.z, box->first.y + pad}, saturate(color::attention(), fractional).alpha(clr_a),
					XOR_STR_UTF8("DUCK"), FNV1A("small8"), gui_legacy::horizontal_alignment::left);
			pad += 9;
		}

		if (cf->test(cfg_t::esp_warn_zeus) && player->get_ammo_count(FNV1A("AMMO_TYPE_TASERCHARGE")))
		{
			draw.text({ box->second.x + padding.z, box->first.y + pad }, saturate(color::warning(), fractional).alpha(clr_a),
					XOR_STR_UTF8("ZEUS"), FNV1A("small8"), gui_legacy::horizontal_alignment::left);
			pad += 9;
		}
		...
		if (cf->test(cfg_t::esp_flags))
		{
			...
			{
				draw.text({ box->second.x + padding.z, box->first.y + pad }, saturate(color::warning(), fractional).alpha(clr_a),
						  XOR_STR_UTF8("C4"), FNV1A("small8"), gui_legacy::horizontal_alignment::left);
				pad += 9;
			}
			...
			{
				draw.text({ box->second.x + padding.z, box->first.y + pad }, saturate(color::warning(), fractional).alpha(clr_a),
						  XOR_STR_UTF8("HOST"), FNV1A("small8"), gui_legacy::horizontal_alignment::left);
				pad += 9;
			}

			if (player->has_defuser())
			{
				draw.text({ box->second.x + padding.z, box->first.y + pad },saturate(color::armor(), fractional).alpha(clr_a),
						XOR_STR_UTF8("D"), FNV1A("small8"), gui_legacy::horizontal_alignment::left);
				pad += 9;
			}

			if (player->get_armor() > 0)
			{
				draw.text({ box->second.x + padding.z, box->first.y + pad }, color::white().alpha(clr_a), player->has_helmet() ?
					XOR_STR_UTF8("KH") : XOR_STR_UTF8("K"), FNV1A("small8"), gui_legacy::horizontal_alignment::left);
				pad += 9;
			}

			if (player_entry.hittable && rag.is_active())
			{
				draw.text({ box->second.x + padding.z, box->first.y + pad }, saturate(color::info(), fractional).alpha(clr_a),
						XOR_STR_UTF8("HIT"), FNV1A("small8"), gui_legacy::horizontal_alignment::left);
				pad += 9;
			}
		}
		...
		if (wpn && wpn->get_clip1() > 0)
		{
			...
			{
				const auto current_ammo = std::clamp(wpn->get_clip1(), 0, wpn_info->maxclip1) / static_cast<float>(wpn_info->maxclip1);
				linear_fade(player_entry.visuals.previous_ammo, current_ammo, 1.f, 1.f, false);

				if (cf->test(cfg_t::esp_ammo))
				{
					draw_bar(draw, ammo_bar, saturate(color::ammo(), fractional).alpha(clr_a), current_ammo, player_entry.visuals.previous_ammo);
					padding.y += 5;
				}
			}
		}
		...
		if (wpn && cf->test(cfg_t::esp_weapon))
		{
			if (cfg.player_esp.prefer_icons.get())
				draw.weapon({ box->first.x + (box->second.x - box->first.x) / 2.f, box->second.y + padding.y + 3 },
					12.f, color::white().alpha(clr_a), wpn->get_icon_name());
			else
			{
				...
				draw.text({ box->first.x + (box->second.x - box->first.x) / 2.f, box->second.y + padding.y + 3 }, color::white().alpha(clr_a),
						  weapon_name.c_str(), FNV1A("small8"), gui_legacy::horizontal_alignment::center);
			}
		}
	}

	void esp::draw_player_offscreen(const gui_legacy::draw_adapter& draw, cs_player_t* const player, const color& clr) const
	{
		...
		auto& player_entry = GET_PLAYER_ENTRY(player);
		float rotation;
		const auto vertex_pos = rotate_offscreen(player_entry.visuals.pos.current, cfg.player_esp.offscreen_radius.get(), rotation);
		...
		draw.polygon(vertex_pos, verts, -rotation, clr.alpha(player_entry.visuals.alpha));
		...
		if (cf->test(cfg_t::esp_health_bar))
		{
			...
			draw_bar(draw, health_bar, interpolate(color::health_end(), color::health_start(), current_health,
					(player_entry.visuals.alpha - .3f) * 2.5f).alpha(player_entry.visuals.alpha), current_health, player_entry.visuals.previous_health,
							cf.get().test(cfg_t::esp_health_text) && current_health <= .99f);
		}
		...
		{
			if (cfg.player_esp.prefer_icons.get())
				draw.weapon({ top_left.x + (bot_right.x - top_left.x) / 2.f, bot_right.y + 6.f },
						12.f, color::white().alpha(player_entry.visuals.alpha), wpn->get_icon_name());
			else
			{
				auto weapon_name = wpn->get_localized_name();
				std::transform(weapon_name.begin(), weapon_name.end(), weapon_name.begin(), ::towupper);
				...
				draw.text({ top_left.x + (bot_right.x - top_left.x) / 2.f, bot_right.y }, color::white().alpha(player_entry.visuals.alpha),
						  weapon_name.c_str(), FNV1A("small8"), gui_legacy::horizontal_alignment::center);
			}
		}
	}
	...
	void esp::draw_entity(const gui_legacy::draw_adapter& draw, entity_t* const ent) const
	{
		...
		{
		case entity_type::c4:
			if (game->client_entity_list->get_client_entity_from_handle(reinterpret_cast<entity_t*>(ent)->get_owner_entity()))
				return;
		...
		case entity_type::projectile:
		{
			...
			auto owner = reinterpret_cast<cs_player_t*>(game->client_entity_list->get_client_entity_from_handle(wpn->get_owner_entity()));
			player_info player_inf;

			if (wpn->get_origin().is_zero() ||
				!owner ||
			...
			if (wpn->get_class_id() == class_id::molotov_projectile && (draw_nade_impact(draw, wpn, true) - vec2(screen_origin.x, screen_origin.y)).length() < 26.f)
				return;

			if (wpn->get_class_id() == class_id::base_csgrenade_projectile && model && *reinterpret_cast<uint32_t*>(model->hdr->name + 13) == 0x67617266
			...
			if (model)
			{
				...
				{
					if (wpn->get_class_id() != class_id::base_csgrenade_projectile || *reinterpret_cast<uint32_t*>(model->hdr->name + 13) != 0x67617266 || wpn->get_grenade_velocity().length() > .1f)
						draw.weapon(vec2(screen_origin.x, screen_origin.y), 10.f, color::white(), wpn->get_nade_icon_name());
				}
				else
			...
			if (cf->get().test(cfg_t::esp_expiry))
			{
				...
				if (start_time > 0.f && stop_time > 0.f)
				{
					...
					const auto timer_bar = std::make_pair(vec2(screen_origin.x - 12, screen_origin.y - 2), vec2(screen_origin.x + 12, screen_origin.y + 2));
					draw_bar(draw, timer_bar, clr->get(), 1.f - (game->globals->curtime - start_time) / stop_time, 1.f - (game->globals->curtime - start_time) / stop_time);
					
					swprintf_s(timer, 256, XOR_STR_UTF8("%.2f"), time_left);
					draw.text(vec2(screen_origin.x, screen_origin.y + 3.f), clr->get(), timer, FNV1A("small8"), gui_legacy::horizontal_alignment::center);
				}
			}

			if (time_left == 0.f && cf->get().test(cfg_t::esp_text))
				draw.text(vec2(screen_origin.x, screen_origin.y), clr->get(), out, FNV1A("small8"), gui_legacy::horizontal_alignment::center);
		...
		case entity_type::weapon_entity:
		{
			...
			if (cf->get().test(cfg_t::esp_text))
			{
				if (cfg.player_esp.prefer_icons.get())
				{
					draw.weapon(vec2(screen_origin.x, screen_origin.y), 10.f, color::white(), wpn->get_icon_name());
					pad += 3;
				}
				else
				{
			...
			{
				const auto percent = std::clamp(wpn->get_clip1(), 0, wpn_info->maxclip1) / static_cast<float>(wpn_info->maxclip1);
				const auto armor_bar = std::make_pair(vec2(screen_origin.x - 12, screen_origin.y - 7), vec2(screen_origin.x + 12, screen_origin.y - 3));
				draw_bar(draw, armor_bar, color::ammo().alpha(clr->get().a() / 255.f), percent, percent);
			}
			break;
		}
		default:
	...
	{
		const auto thrower = reinterpret_cast<cs_player_t*>(game->client_entity_list->get_client_entity_from_handle(wpn->get_thrower()));
		const auto progress = wpn->get_next_trail_line();
		if (!game->local_player || !thrower || !thrower->is_player() || !cfg.world_esp.grenade_warning.get() || !wpn->get_studio_hdr()
			|| wpn->get_last_trail_pos().z == FLT_MAX || progress > 1.f || progress < 0.f
			|| wpn->get_simulation_time() < TICKS_TO_TIME(game->client_state->last_server_tick) - .35f)
			return vec2(FLT_MAX, FLT_MAX);

		if (cfg.world_esp.grenade_warning_options->test(cfg_t::grenade_warning_only_hostile)
			&& (GET_CONVAR_INT("mp_friendlyfire") || (!game->local_player->is_enemy(thrower) && game->local_player != thrower)))
			return vec2(FLT_MAX, FLT_MAX);
		...
		{
			float rotation;
			screen = rotate_offscreen(wpn->get_last_trail_pos(), .5f * evo::ren::adapter->display.x - 48.f, rotation);
		}
		else
		...
		if (cfg.world_esp.grenade_warning_options->test(cfg_t::grenade_warning_show_type))
			...
			draw.text(screen, color::white().alpha(alpha), L"!", FNV1A("segoeuib28"), gui_legacy::horizontal_alignment::center, gui_legacy::vertical_alignment::center);

		return screen;
	}

	void esp::draw_box(const gui_legacy::draw_adapter& draw, const std::pair<gui_legacy::vec2, gui_legacy::vec2>& box, const sdk::color& clr, const bool drop_shadow) const
	{
		if (drop_shadow)
		{
	...
	void esp::draw_bar(const gui_legacy::draw_adapter& draw, const std::pair<gui_legacy::vec2, gui_legacy::vec2>& box, const sdk::color& clr, const float percent, const float old_percent, bool text) const
	{
		...
		if (is_horizontal)
		{
			...
			draw.rect_filled(box.first + vec2(end, 1), box.first + vec2(old_end, box.second.y - box.first.y - 1), clr.alpha(.45f));
		}
		else
		{
			draw.rect_filled(box.first + vec2(1, 1 + length - end), box.second - vec2(1, 1), clr);
			draw.rect_filled(box.first + vec2(1, 1 + length - old_end), box.first + vec2(box.second.x - box.first.x - 1, length - end), clr.alpha(.45f));
		}
		...
		if (text)
		{
			...
			if (is_horizontal)
				draw.text(box.first + vec2(end, box.second.y - box.first.y - 1), cl, XOR_FORMAT_UTF8("%i", percent * 100), FNV1A("small8"), gui_legacy::horizontal_alignment::center, gui_legacy::vertical_alignment::center);
			else
				draw.text(box.first + vec2(2, length - end + 1), cl, XOR_FORMAT_UTF8("%i", percent * 100), FNV1A("small8"), gui_legacy::horizontal_alignment::center, gui_legacy::vertical_alignment::center);
		}
	}
	...
	void esp::draw_bones(const gui_legacy::draw_adapter& draw, cs_player_t* player, const color& clr) const
	{
		...
		for (auto i = 0; i < hdr->numbones(); i++)
		{
			...
			auto to = bone->parent;
			for (auto to_bone = hdr->bone(to); (!(bone->flags & bone_used_by_hitbox) || !has_hitbox_attached(to)) && to_bone; to = to_bone->parent);
			...
			draw.line(vec2(screen_from.x, screen_from.y), vec2(screen_to.x, screen_to.y), clr.alpha(entry.visuals.dormant_alpha));
		}
	}

	std::wstring esp::get_player_name(cs_player_t* const player) const
	{
		const auto info = game->engine_client->get_player_info(player->index());
		auto name = util::utf8_decode(info.name);
		shorten_str(name);
		return name;
	}
	...
	float esp::estimate_nade_damage_fraction(cs_player_t* const thrower, cs_player_t* const player, const vec3& impact, bool molotov) const
	{
		...
		if ((world_space_center - impact).length() < 350.f * GET_CONVAR_FLOAT("sv_hegrenade_radius_multiplier"))
		{
			...
			if (tr.fraction >= .99f)
			{
				...
				const auto scaled_damage = max(int32_t(ceilf(trace.scale_damage(player, unscaled_damage, .5f, hitgroup_generic))), 0);
				...
				{
					if (thrower == player)
						return (scaled_damage * GET_CONVAR_FLOAT("ff_damage_reduction_grenade_self")) / float(player->get_player_health());

					return (scaled_damage * GET_CONVAR_FLOAT("ff_damage_reduction_grenade")) / float(player->get_player_health());
				}

				return (scaled_damage * GET_CONVAR_FLOAT("sv_hegrenade_damage_multiplier")) / float(player->get_player_health());
			}
		}
	...
	std::optional<std::pair<gui_legacy::vec2, gui_legacy::vec2>> esp::get_player_bounds(cs_player_t* const player) const
	{
		...
		const auto shoot = player->is_dormant() ?
		...
		auto box = std::make_pair(vec2(median_x - half_width, median_y - half_height), vec2(median_x + half_width, median_y + half_height));
		normalize_box(box);
		return box;
	}
	...
	{
		const auto half_x = .5f * evo::ren::adapter->display.x;
		const auto half_y = .5f * evo::ren::adapter->display.y;
	...
	vec2 esp::rotate_offscreen(vec3 pos, float offset, float& rotation) const
	{
		...
		const auto screen_size = evo::ren::adapter->display, screen_center = screen_size / 2.f;
		...
		return vec2(screen_center.x + offset * sin(rotation_rad),
				screen_center.y - offset_y * cos(rotation_rad));
	}
}
