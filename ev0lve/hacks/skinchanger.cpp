...
namespace hacks
{
	...
	void skinchanger::on_player_death(sdk::game_event* event)
	{
		...
		if (event_weapon_name.find(XOR_STR("knife")) == std::string::npos && event_weapon_name.find(XOR_STR("bayonet")) == std::string::npos)
			return;
	...
	void skinchanger::on_post_data_update()
	{
		...
		const auto player = reinterpret_cast<cs_player_t*>(game->client_entity_list->get_client_entity(game->engine_client->get_local_player()));
	...
	void skinchanger::update_hud()
	{
		...
		for (int32_t j = 0; j < *hud_weapons->get_weapon_count(); j++)
			j = reinterpret_cast<int32_t(__thiscall *)(void *, int32_t)>(game->client.at(functions::clear_hud_weapon_icon))(hud_weapons, j);
	...
	bool skinchanger::weapon_has_skin(int32_t index)
	{
		...
		return (skin_ct.index == index && skin_ct.data.paint_kit > 0) || (skin_t.index == index && skin_t.data.paint_kit > 0);
	}
	...
	void skinchanger::equip(int32_t index, int32_t team, int32_t paint_kit, float wear, int32_t seed, int32_t stattrak, bool dyn_stattrak)
	{
	...
	void skinchanger::parse_game_items()
	{
		...
		for (int iterator = 0; iterator < items->max_element(); ++iterator)
		{
			...
			if (prefabHash != FNV1A("melee")
				...
				&& prefabHash != FNV1A("customplayer")
				&& prefabHash != FNV1A("customplayertradable"))
				continue;
			...
			if (prefabHash == FNV1A("customplayer") || prefabHash == FNV1A("customplayertradable"))
			{
				...
				{
					if (std::string(elem->item_base_name).find(XOR("_t")) != std::string::npos || (uint16_t(elem->def_index) >= 5093 && uint16_t(elem->def_index) <= 5096))
						weapon_infos[int32_t(-2)].paint_kits.emplace_back(
								paint_kit_data_s{int32_t(elem->def_index), elem->item_rarity, localized_name});
					else if (std::string(elem->item_base_name).find(XOR("_ct")) != std::string::npos || uint16_t(elem->def_index) == 5097)
						weapon_infos[int32_t(-3)].paint_kits.emplace_back(
								paint_kit_data_s{int32_t(elem->def_index), elem->item_rarity, localized_name});
				}
			}

			models[int32_t(elem->def_index)] = std::make_tuple(std::string(elem->definition_name), std::string(elem->base_display_model),
		...
		for (auto i = 0; i < paint_kits->max_element(); ++i)
		{
			...
			skin_ids.emplace(std::pair<uint32_t, std::pair<int32_t, std::string>>(elem->id, std::pair<int32_t, std::string>(elem->rarity, localized_name)));
		}
		...
		for (auto i = 0; i < item_sets->max_element(); ++i)
		{
			...
			for (auto y = 0; y < item_set->item_entries.count(); ++y)
			{
				...
				if (item.item_def <= 0 || item.item_def > size_t(item_definition_index::studded_hydra_gloves)
					|| (item.item_def > size_t(item_definition_index::weapon_knife_skeleton) && item.item_def <= size_t(item_definition_index::studded_bloodhound_gloves)))
					continue;
		...
		{
			// NOTE: howl fix
			weapon_infos[size_t(item_definition_index::weapon_m4a1)].paint_kits.emplace_back(paint_kit_data_s { 309, 7, skin_ids[309].second });

			// NOTE: all of those listed weapons have fucked rarities
			std::vector<item_definition_index> items_to_fix =
			{
					item_definition_index::weapon_deagle,
					item_definition_index::weapon_glock,
					item_definition_index::weapon_ak47,
					item_definition_index::weapon_awp,
					item_definition_index::weapon_m4a1,
					item_definition_index::weapon_hkp2000,
					item_definition_index::weapon_m4a1_silencer,
					item_definition_index::weapon_usp_silencer
			};
		...
		models[int32_t(-2)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_hoxton.mdl"));
		models[int32_t(-3)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/porcelain_doll.mdl"));
		models[int32_t(-4)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_skull.mdl"));
		models[int32_t(-5)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_samurai.mdl"));
		models[int32_t(-6)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/evil_clown.mdl"));
		models[int32_t(-7)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_wolf.mdl"));
		models[int32_t(-8)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_sheep_model.mdl"));
		models[int32_t(-9)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_bunny_gold.mdl"));
		models[int32_t(-10)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_anaglyph.mdl"));
		models[int32_t(-11)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_porcelain_doll_kabuki.mdl"));
		models[int32_t(-12)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_dallas.mdl"));
		models[int32_t(-13)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_pumpkin.mdl"));
		models[int32_t(-14)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_sheep_bloody.mdl"));
		models[int32_t(-15)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_devil_plastic.mdl"));
		models[int32_t(-16)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_boar.mdl"));
		models[int32_t(-17)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_chains.mdl"));
		models[int32_t(-18)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_tiki.mdl"));
		models[int32_t(-19)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_bunny.mdl"));
		models[int32_t(-20)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_sheep_gold.mdl"));
		models[int32_t(-21)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_zombie_fortune_plastic.mdl"));
		models[int32_t(-22)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_chicken.mdl"));
		models[int32_t(-23)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_skull_gold.mdl"));
		models[int32_t(-24)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_tf2_demo_model.mdl"));
		models[int32_t(-25)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_tf2_engi_model.mdl"));
		models[int32_t(-26)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_tf2_heavy_model.mdl"));
		models[int32_t(-27)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_tf2_medic_model.mdl"));
		models[int32_t(-28)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_tf2_pyro_model.mdl"));
		models[int32_t(-29)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_tf2_scout_model.mdl"));
		models[int32_t(-30)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_tf2_sniper_model.mdl"));
		models[int32_t(-31)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_tf2_soldier_model.mdl"));
		models[int32_t(-32)] = std::make_tuple(std::string(), std::string(), XOR_STR("models/player/holiday/facemasks/facemask_tf2_spy_model.mdl"));

		weapon_infos[int32_t(-4)].paint_kits =
		{
			{ -2, 1, XOR_STR("Hoxton") },
			{ -3, 1, XOR_STR("Porcelain Doll") },
			{ -4, 1, XOR_STR("Skull") },
			{ -5, 1, XOR_STR("Samurai") },
			{ -6, 1, XOR_STR("Evil Clown") },
			{ -7, 1, XOR_STR("Wolf") },
			{ -8, 1, XOR_STR("Sheep") },
			{ -9, 1, XOR_STR("Bunny Gold") },
			{ -10, 1, XOR_STR("Anaglyph") },
			{ -11, 1, XOR_STR("Porcelain Doll Kabuki") },
			{ -12, 1, XOR_STR("Dallas") },
			{ -13, 1, XOR_STR("Pumpkin") },
			{ -14, 1, XOR_STR("Sheep Bloody") },
			{ -15, 1, XOR_STR("Devil Plastic") },
			{ -16, 1, XOR_STR("Boar") },
			{ -17, 1, XOR_STR("Chains") },
			{ -18, 1, XOR_STR("Tiki") },
			{ -19, 1, XOR_STR("Bunny") },
			{ -20, 1, XOR_STR("Sheep Gold") },
			{ -21, 1, XOR_STR("Zombie Fortune Plastic") },
			{ -22, 1, XOR_STR("Chicken") },
			{ -23, 1, XOR_STR("Skull Gold") },
			{ -24, 1, XOR_STR("TF2 Demoman") },
			{ -25, 1, XOR_STR("TF2 Engineer") },
			{ -26, 1, XOR_STR("TF2 Heavy") },
			{ -27, 1, XOR_STR("TF2 Medic") },
			{ -28, 1, XOR_STR("TF2 Pyro") },
			{ -29, 1, XOR_STR("TF2 Scout") },
			{ -30, 1, XOR_STR("TF2 Sniper") },
			{ -31, 1, XOR_STR("TF2 Soldier") },
	...
	void skinchanger::fill_weapon_list()
	{
		...
		ctx->find<layout>(GUI_HASH("skins.settings_content"))->set_visible(false);
		...
		for (const auto& wpn : weapon_infos)
		{
			...
			const auto sel = std::make_shared<selectable>(gui::hash(wpn.second.item_name),
														  wpn.second.name);
			sel->value = wpn.first;
	...
	void skinchanger::fill_skin_list(int id, std::string& search)
	{
		...
		for (auto& skins : weapon_infos)
		{
			...
			if (cfg.skins.sort_by_item.get() && skins.first != id)
				continue;
			...
			for (auto& skin: skins.second.paint_kits)
			{
				...
				const auto sel = std::make_shared<selectable>(
						gui::hash(skin.name + std::to_string(skin.rarity) + std::to_string(skin.paint_kit) + skins.second.item_name),
						skin.name,
						rarity_colors[skin.rarity == 7 ? skin.rarity : skin.rarity - 1]);
				...
				// TODO: make nicer
				if (skin_ct.data.paint_kit == skin.paint_kit &&
					skin_ct.index == id)
					text_addition += XOR(" (CT)");

				if (skin_t.data.paint_kit == skin.paint_kit &&
					skin_t.index == id)
					text_addition += XOR(" (T)");
				...
				auto skin_not_in_list = std::find_if(skins_to_add.begin(), skins_to_add.end(),
													 [&](std::shared_ptr<selectable> &s) { return s->value == skin.paint_kit; }) == skins_to_add.end();
		...
		std::sort(skins_to_add.begin(), skins_to_add.end(), [](const std::shared_ptr<selectable>& l, const std::shared_ptr<selectable>& r){
			if(l->order < r->order) return true;
			if(l->order > r->order) return false;
	...
	void skinchanger::apply_skin(weapon_t* weapon, skin_data_s* skin, const int owner_id)
	{
		...
		const auto wear_changed = weapon->get_fallback_wear() != max(skin->wear, 0.001f);
		const auto stattrak_changed = weapon->get_fallback_stat_trak() != skin->stattrak;
		const auto stattrak_enable_changed = (skin->stattrak == -1 || weapon->get_fallback_stat_trak() == -1) && stattrak_changed;
		...
		if (should_update)
		{
			...
			// weapon_t id of skin doesn't match actual weapon_t id
			...
			weapon->get_custom_materials().remove_all();
			weapon->get_econ_item_view()->get_custom_materials().remove_all();
			weapon->get_econ_item_view()->get_visual_data_processors().remove_all();
	...
	void skinchanger::apply_glove()
	{
		...
		const auto glove = reinterpret_cast<weapon_t*>(game->client_entity_list->get_client_entity_from_handle(player->get_my_wearables()[0]));
		...
		glove->get_fallback_wear() = max(skin->wear, 0.001f);
		...
		if (skin->index != size_t(glove->get_item_definition_index()))
		{
			...
			const auto viewmodel_entity = reinterpret_cast<entity_t*>(game->client_entity_list->get_client_entity_from_handle(player->get_view_model_0()));
			if (!viewmodel_entity)
				return;
	...
	void skinchanger::remove_glove()
	{
		...
		const auto player = reinterpret_cast<cs_player_t*>(game->client_entity_list->get_client_entity(game->engine_client->get_local_player()));
		const auto our_glove = reinterpret_cast<weapon_t*>(game->client_entity_list->get_client_entity_from_handle(glove_handle));
	...
	void skinchanger::handle_player_model()
	{
		...
		auto& target_model = player->get_team_num() == 2 ? equipped_model_t.data.paint_kit : equipped_model_ct.data.paint_kit;
		...
		else if (target_model > 0)
		{
			...
			if (model <= 0)
			{
				...
				const auto arm_cfg = ((viewmodel_arm_config*(__thiscall*)(const char*)) game->client.at(functions::get_player_viewmodel_arm_config_for_player_model))(model_name.c_str());
				if (arm_cfg)
				{
		...
		if (!player->is_alive())
		{
			const auto ragdoll = player->get_ragdoll();
	...
	{
		for (const auto& handle : game->local_player->get_my_weapons())
		{
			...
			const auto weapon = reinterpret_cast<weapon_t*>(game->client_entity_list->get_client_entity_from_handle(handle));
			...
			const auto skin = &get_skin_data(definition_index, game->local_player->get_team_num());
	...
	{
		// get viewmodel
		const auto viewmodel_entity = reinterpret_cast<weapon_t*>(game->client_entity_list->get_client_entity_from_handle(game->local_player->get_view_model_0()));
		if (!viewmodel_entity)
			return;

		// get viewmodel weapon_t if it's a knife
		auto weapon_entity = reinterpret_cast<weapon_t*>(game->client_entity_list->get_client_entity_from_handle(viewmodel_entity->get_viewmodel_weapon()));
		if (!weapon_entity || !is_knife_by_index(int32_t(weapon_entity->get_item_definition_index())))
			return;

		update_sequence_table(viewmodel_entity);

		// update model index
		const auto view_model_index = game->model_info_client->get_model_index(std::get<1>(models[size_t(weapon_entity->get_item_definition_index())]).c_str());
		const auto world_model_index = game->model_info_client->get_model_index(std::get<2>(models[size_t(weapon_entity->get_item_definition_index())]).c_str());
		...
		viewmodel_entity->get_model_index() = view_model_index;

		// TODO: fix this so animations work properly
		/*if (weapon_entity->get_model_index() != knife_model_index + 1)
			weapon_entity->set_model_index(knife_model_index + 1);*/

		// update world model
		auto weapon_world_entity = reinterpret_cast<weapon_t*>(game->client_entity_list->get_client_entity_from_handle(weapon_entity->get_weapon_world_model()));
	...
	void skinchanger::update_gui_components()
	{
		...
		equip_ct->set_visible(available_teams == 1 || available_teams == 3);
		equip_t->set_visible(available_teams == 1 || available_teams == 2);
		equip_both->set_visible(available_teams == 1);
		...
		ctx->find<layout>(GUI_HASH("skins.settings.stattrak_content"))->set_visible(stattrak_type != 0 && selected_type <= 3);
