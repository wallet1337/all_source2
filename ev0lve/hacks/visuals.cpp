...
namespace hacks
{
	void visuals::on_player_hurt(game_event* event)
	{
		...
		const auto target = reinterpret_cast<cs_player_t*>(game->client_entity_list->get_client_entity(game->engine_client->get_player_for_user_id(event->get_int(XOR_STR("userid")))));
		const auto attacker = reinterpret_cast<cs_player_t*>(game->client_entity_list->get_client_entity(game->engine_client->get_player_for_user_id(event->get_int(XOR_STR("attacker")))));

		// invalid or not our shot
		if (!attacker || !target || attacker->index() != game->engine_client->get_local_player() || !target->is_enemy())
			return;

		hitmarker_color = (event->get_int(XOR_STR("health")) <= 0 && cfg.local_visuals.hitmarker_style->test(cfg_t::hitmarker_animated))
				? color::attention()
				: color::white();
		hitmarker_offset = 0.f;
		hitmarker_alpha = 255.f;

		if (cfg.local_visuals.hitmarker.get())
		{
	...
	void visuals::on_frame_render_start(const bool pre) const
	{
		...
		{
			const auto lp = reinterpret_cast<cs_player_t*>(game->client_entity_list->get_client_entity(game->engine_client->get_local_player()));
	...
	void visuals::on_post_data_update() const
	{
		...
		game->client_entity_list->for_each([](client_entity* const ent)
		{
			// all entities with models inherit from baseanimating.
			if (!ent->get_model())
				return;

			const auto e = reinterpret_cast<entity_t*>(ent);
			const auto owner = reinterpret_cast<entity_t*>(game->client_entity_list->get_client_entity_from_handle(e->get_owner_entity()));

			if ((owner && owner->is_player()) || ent->get_class_id() == class_id::csragdoll)
	...
	{
		game->client_entity_list->for_each_player([](cs_player_t* const player) -> void
		{
	...
	void visuals::third_person(view_setup* const setup)
	{
		...
		if (game->local_player)
		{
			if (cfg.local_visuals.thirdperson.get())
			{
				...
				if (!player->is_alive())
					player = reinterpret_cast<cs_player_t*>(game->client_entity_list->get_client_entity_from_handle(player->get_observer_target()));

				if (!player)
					return;

				auto adjusted_eye = aa.get_visual_eye_position();
				game->input->camera_offset = vec3(setup->angles.x, setup->angles.y, cfg.local_visuals.thirdperson_distance.get() * third_person_dist);
	...
	void visuals::set_alpha_for_third_person(const model_render_info_t& info) const
	{
		...
		if (!view_setup || !game->local_player || !game->local_player->is_alive() || !current || !current->is_player() || !game->input->camera_in_third_person)
			return;

		const auto local_weapon = reinterpret_cast<weapon_t*>(game->client_entity_list->get_client_entity_from_handle(game->local_player->get_active_weapon()));
		if (current == game->local_player && (!local_weapon || !local_weapon->is_scoped_weapon() || !local_weapon->get_zoom_level()))
			return;
		...
		auto& cf = current == game->local_player ? cfg.local_visuals.local_chams : (current->is_enemy() ? cfg.player_esp.enemy_chams : cfg.player_esp.team_chams);
		auto alpha_scale = cfg.local_visuals.local_chams.mode.get().test(cfg_t::chams_disabled) ? 1.f : cf.primary.get().a() / 255.f;

		if (cfg.local_visuals.glow_local.get())
			alpha_scale *= 1.f - cfg.local_visuals.local_color_glow.get().a() / 255.f * .6f;
	...
	void visuals::indicators(const gui_legacy::draw_adapter& draw)
	{
		...
		if (!cfg.local_visuals.thirdperson.get())
		{
			...
			const vec2 screen_size = vec2(evo::ren::adapter->display.x, evo::ren::adapter->display.y), screen_center = screen_size / 2.f;
			...
			{
				const auto rotation_real = std::remainderf(game->engine_client->get_view_angles().y - game->local_player->get_anim_state()->foot_yaw, yaw_bounds);
				const float rotation_rad_real = DEG2RAD(rotation_real);
				const auto pos_real = vec2(screen_center.x + offset_x * sin(rotation_rad_real), screen_center.y - offset_y * cos(rotation_rad_real));
			...
			{
				const auto rotation_desync = std::remainderf(game->engine_client->get_view_angles().y - local_fake_record.state[resolver_networked].foot_yaw, yaw_bounds);
				const float rotation_rad_desync = DEG2RAD(rotation_desync);
				const auto pos_desync = vec2(screen_center.x + offset_x * sin(rotation_rad_desync), screen_center.y - offset_y * cos(rotation_rad_desync));
	...
	{
		linear_fade(hitmarker_alpha, 0.f, 255.f, 255.f / (cfg.local_visuals.hitmarker_style->test(cfg_t::hitmarker_animated) ? .4f : .8f), false);
		...
		const auto center = vec2(evo::ren::adapter->display.x, evo::ren::adapter->display.y) / 2.f;
		...
		{
			draw.line(vec2(center.x - 9 - offset, center.y - 9 - offset), vec2(center.x - 3 - offset, center.y - 3 - offset), hitmarker_color.alpha(alpha), 2.f);
			draw.line(vec2(center.x - 9 - offset, center.y + 9 + offset), vec2(center.x - 3 - offset, center.y + 3 + offset), hitmarker_color.alpha(alpha), 2.f);
			draw.line(vec2(center.x + 9 + offset, center.y + 9 + offset), vec2(center.x + 3 + offset, center.y + 3 + offset), hitmarker_color.alpha(alpha), 2.f);
			draw.line(vec2(center.x + 9 + offset, center.y - 9 - offset), vec2(center.x + 3 + offset, center.y - 3 - offset), hitmarker_color.alpha(alpha), 2.f);
		} else
	...
	void visuals::scope(const gui_legacy::draw_adapter& draw) const
	{
		...
		if (!wpn || !wpn->is_scoped_weapon() || wpn->get_zoom_level() == 0)
			return;

		if ((wpn->get_item_definition_index() == item_definition_index::weapon_sg556 || wpn->get_item_definition_index() == item_definition_index::weapon_aug)
				&& !game->input->camera_in_third_person)
			return;

		const auto spread = std::clamp((wpn->get_inaccuracy() - wpn->get_cs_weapon_data()->flinaccuracystandalt) * 5.f, 0.f, 1.f);
		const auto w = evo::ren::adapter->display.x;
		const auto h = evo::ren::adapter->display.y;
		...
		else if (cfg.removals.scope->test(cfg_t::scope_small) || cfg.removals.scope->test(cfg_t::scope_small_dynamic))
		{
			...
			const auto clr = color(cfg_clr.r(), cfg_clr.g(), cfg_clr.b(), cfg_clr.a()).alpha(alpha);
			const auto offset = cfg.removals.scope->test(cfg_t::scope_small_dynamic) ? spread * 40: 0;
			draw.rect_filled_fade(vec2(w / 2 - width / 2, h / 2 - 100  - offset), vec2(w / 2 + width / 2, h / 2 - 10 - offset), clr.alpha(0.f), clr, true);
			draw.rect_filled_fade(vec2(w / 2 - width / 2, h / 2 + 10 + offset), vec2(w / 2 + width / 2, h / 2 + 100 + offset), clr, clr.alpha(0.f), true);
			draw.rect_filled_fade(vec2(w / 2 - 100 - offset, h / 2 - width / 2), vec2(w / 2 - 10 - offset, h / 2 + width / 2), clr.alpha(0.f), clr);
			draw.rect_filled_fade(vec2(w / 2 + 10 + offset, h / 2 - width / 2), vec2(w / 2 + 100 + offset, h / 2 + width / 2), clr, clr.alpha(0.f));
		}
		else
		{
			const auto clr = color::black().alpha(alpha);
	...
	void visuals::pen_crosshair(const gui_legacy::draw_adapter& draw) const
	{
		...
		auto wpn = reinterpret_cast<weapon_t*>(game->client_entity_list->get_client_entity_from_handle(game->local_player->get_active_weapon()));

		if (!wpn || wpn->is_knife() || wpn->is_grenade() || wpn->get_item_definition_index() == item_definition_index::weapon_taser)
			return;

		const auto center = vec2(evo::ren::adapter->display.x, evo::ren::adapter->display.y) / 2.f;
		draw.rect_filled(center - vec2(2, 2), center + vec2(2, 2), trace.can_wallbang() ? color(0, 255, 0, 255).alpha(.7f) : color(255, 0, 0, 255).alpha(.7f));
	...
	void visuals::output_eventlog(const gui_legacy::draw_adapter& draw) const
	{
		...
		for (size_t i = 0; i < text_log.size(); ++i)
		{
			...
			if (expire_time <= game->globals->realtime || std::get<2>(log_line) <= .1f)
				...
				draw.text(vec2(5, 5 + i * (line_size.y + 2)), color::white().alpha(std::get<2>(log_line) * draw.get_alpha_modifier()),
