...
namespace hacks
{
	...
	void peek_assistant_t::on_create_move(user_cmd* cmd)
	{
		const auto track_glowies = [this](const vec3 visual_pos)
		{
			...
			for (auto i = 0; i < radius + 1; i++)
			{
				...
				if (delta_pos.length2d() < .25f * .33f * radius)
				{
					...
					game->glow_manager->add_glow_box(seg, trajectory, { 0.f, -.2f, -.2f }, { delta_pos.length(), .2f, .2f }, clr, .5f);
				}
				last_seg = seg;
			}
		...
		const auto vel = game->local_player->get_velocity();
		if (game->cs_game_movement->process_movement(game->local_player, &data); (npos - *pos).length2d() > 1.1f && vel.length2d() > data.velocity.length2d())
		{
			auto vel_dir = data.velocity - vel;
			vel_dir.z = 0.f;
			vel_dir.normalize();

			auto pos_dir = start.abs_origin - shot_pos;
			pos_dir.z = 0.f;
			pos_dir.normalize();

			if (vel_dir.dot(pos_dir) > 0.f)
			{
				if (!state)
					pos = std::nullopt;
				has_shot = false;
	...
	void peek_assistant_t::on_post_move(user_cmd* cmd)
	{
		...
		const auto start = game->local_player->get_eye_position() + game->local_player->get_velocity()
				* game->globals->interval_per_tick * max(0, TIME_TO_TICKS(.1f) - 2);
