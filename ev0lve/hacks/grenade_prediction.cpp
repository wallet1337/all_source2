...
namespace hacks
{
	...
	void grenade_prediction::draw()
	{
		...
		if (landed)
		{
			...
			{				
				if (!in_segment(i, segment_1_start, segment_1_end)
					&& !in_segment(i, segment_2_start, segment_2_end))
					continue;
				...
				if (delta_pos.length2d() < .25f * radius)
				{
					...
					game->glow_manager->add_glow_box(seg, trajectory, { 0.f, -.2f, -.2f }, { delta_pos.length(), .2f, .2f }, color(255, 0, 0));
				}
	...
	std::optional<vec3> grenade_prediction::to_end(weapon_t* wpn, cs_player_t* thrower, bool molotov, float* progress)
	{
		...
		if (thrower->is_dormant() && fabsf(wpn->get_spawn_time() - time) < max(1.5f, GET_CONVAR_FLOAT("molotov_throw_detonate_time")))
		...
		if (progress)
			*progress = std::clamp(1.f - TICKS_TO_TIME(steps - i) / TICKS_TO_TIME(TIME_TO_TICKS(will_detonate()) - i), 0.f, 1.f);

		game_trace trace;
		hull_trace(filter, start_pos, start_pos - vec3(0.f, 0.f, 12.5f), trace, mask_solid | contents_current_90 | contents_slime | contents_water);
		return trace.endpos;
	}
	...
	void grenade_prediction::setup(trace_simple_filter& filter, weapon_t* wpn, vec3& vec_src, vec3& vec_throw)
	{
		...
		const auto start_pos = aa.get_visual_eye_position() + vec3(0.f, 0.f, throw_height) + game->local_player->get_velocity() * .1f;
	...
	void grenade_prediction::move_entity(sdk::trace_simple_filter& filter, sdk::vec3& vec_src, const sdk::vec3& move, sdk::game_trace& tr, uint32_t mask)
	{
		const vec3 vec_end = vec_src + move;
		hull_trace(filter,vec_src, vec_end, tr, mask);
	}

	void grenade_prediction::hull_trace(sdk::trace_simple_filter& filter, const sdk::vec3& vec_src, const sdk::vec3& vec_dest, sdk::game_trace& tr, uint32_t mask)
	{
		ray r;
	...
	states grenade_prediction::calculate_path(sdk::trace_simple_filter& filter, sdk::game_trace& tr, sdk::vec3& vec_velocity)
	{
		...
		if (vec_velocity.length2d() <= 20.f &&
			(weapon_id == item_definition_index::weapon_decoy ||
			weapon_id == item_definition_index::weapon_smokegrenade))
			return state_detonate;
