...
namespace gui_legacy
{
	...
	public:
		...
		virtual void rect_filled_fade(const vec2& from, const vec2& to, const color& cl0, const color& cl1, const bool horizontal = false) const = 0;
		...
		virtual void circle_filled_fade(const vec2& center, int32_t radius, const color& cl, float progress = 1.f) const = 0;
		virtual void circle(const vec2& center, int32_t radius, const color& cl, float progress = 1.f, bool filled = true) const = 0;
		virtual void polygon(vec2 pos, std::vector<sdk::vertex_t> vertices, const float rotation, const color& cl) const = 0;
		virtual void text(const vec2& pos, const color& cl, const wchar_t* const text, const uint32_t font = default_font,
			const horizontal_alignment horizontal = horizontal_alignment::left, const vertical_alignment vertical = vertical_alignment::top) const = 0;
