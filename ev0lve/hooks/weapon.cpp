...
namespace hooks::weapon
{
	...
	{
		const auto owner = reinterpret_cast<cs_player_t*>(game->client_entity_list->get_client_entity_from_handle(weapon->get_owner_entity()));
		if (owner && owner->is_player() && owner->is_valid() && owner->index() == game->engine_client->get_local_player())
		{
			...
			if (animation.sequence == context.cmd.command_number - 1 && (fresh || context.cmd.command_number <= game->client_state->last_command))
	...
	{
		if (uintptr_t(_ReturnAddress()) == game->client.at(functions::return_to_update_post_processing)
			&& cfg.local_visuals.fov_changer.get() && game->input->camera_in_third_person
			&& (wpn->get_item_definition_index() == item_definition_index::weapon_sg556 || wpn->get_item_definition_index() == item_definition_index::weapon_aug))
