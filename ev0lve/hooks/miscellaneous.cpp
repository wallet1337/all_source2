...
namespace hooks::miscellaneous
{
	...
	{
		if (game->cs_game_movement->is_processing && uintptr_t(_ReturnAddress()) == game->engine.at(functions::return_to_start_sound_entry))
			return (const char*) 1;
	...
	{
		static constexpr auto offsets = std::array { 0xCE, 0xEB, 0x108, 0x00, 0x70, 0x70, 0x70, 0x70, 0x80, 0x80, 0x80, 0x80, 0x90, 0x90, 0x90, 0x90 };
		...
		if (cfg.local_visuals.night_mode.get())
		{
			...
			tonemap_settings.use_custom_bloom_scale = tonemap_settings.use_custom_auto_exposure_min
				= tonemap_settings.use_custom_auto_exposure_max = true;
			...
			tonemap_settings.custom_bloom_scale = .2f + std::clamp(cfg.local_visuals.night_mode_value.get() * .01f, 0.f, 1.f) * 3.75f;
			...
			for (auto i = 0u; i < offsets.size(); i++)
			{
				...
				*reinterpret_cast<uint8_t*>(*reinterpret_cast<uintptr_t*>(uintptr_t(hook_manager.get_tonemap_settings_from_env_tonemap_controller->get_target()) + offset) + rollback) =
					*reinterpret_cast<uint8_t*>(uintptr_t(&tonemap_settings) + i);
			}
		}
	}

	void __fastcall draw_mesh(uintptr_t dx8_api, uint32_t edx, uint32_t mesh, uint32_t count, mesh_instance_data* instances, uint32_t compression, uint32_t compiled_state, uint32_t info)
	{
		auto& mat = *reinterpret_cast<material**>(dx8_api + globals::shaderapidx8_material_offset);
		
		if (mat && (mat->get_group()[6] == 'P' || (mat->get_name()[7] == 'p' && mat->get_name()[8] == 'r' && mat->get_name()[9] == 'o'
	...
	void __fastcall render_glow_boxes(glow_manager_t* glow, uintptr_t edx, int32_t pass, mat_render_context** ctx)
	{
		...
		game->client_entity_list->for_each_player([&](cs_player_t* player)
		{
			...
			if (!model || !player->is_alive() || !entry.last_target
				|| entry.last_target->record->recv_time + 2.f < game->globals->curtime)
				return;

			const auto hdr = game->model_info_client->get_studio_model(model);
			...
			const auto clr = entry.last_target->pen.secure_point ?
				cfg.player_esp.target_scan_secure.get() :
				cfg.player_esp.target_scan_aim.get();
			const auto rad = max(hitbox->radius, 2.5f);
			const auto pos = player->get_abs_origin() + entry.interpolated_target.advance(.5f * game->globals->frametime);
	...
	void __cdecl convar_network_change_callback(convar* var, const char* old1, float old2)
	{
		...
		if (var->has_flag(1 << 9))
		{
			if (FNV1A_CMP(var->get_base_name(), "cl_lagcompensation"))
				game->cl_lagcompensation = !FNV1A_CMP(var2->string, "0");
			if (FNV1A_CMP(var->get_base_name(), "cl_predict"))
				game->cl_predict = !FNV1A_CMP(var2->string, "0");
		}
	...
	{
		const auto status = hook_manager.nt_query_virtual_memory->call(process_handle, base_address, memory_information_class, mem_information, length, result_length);
		if (!status && !memory_information_class && reinterpret_cast<uintptr_t>(mem_information->AllocationBase) == game->base)
