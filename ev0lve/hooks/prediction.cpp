...
{
	void __fastcall run_command(prediction_t* prediction, uint32_t edx, cs_player_t* player, user_cmd* cmd, uintptr_t helper)
	{
		...
		{
			if (cmd->predicted)
				wpn->get_last_client_fire_bullet_time() = game->globals->curtime;
			else
			{
				zoom_level = wpn->get_zoom_level();
				wpn->get_last_client_fire_bullet_time() = game->globals->curtime - 1.f;
			}
		...
		if (info.sequence == cmd->command_number)
			player->get_tick_base() += info.shift;

		// restore dumb shit.
		auto& last = pred.info[(cmd->command_number - 1) % input_max];
		if (last.sequence == cmd->command_number)
		{
		...
		info.tick_base = player->get_tick_base();
		...
		info.view_delta = GET_PLAYER_ENTRY(player).view_delta;
		...
		if (!(cmd->buttons & user_cmd::reload) && zoom_level > -1
			&& wpn == reinterpret_cast<weapon_t*>(game->client_entity_list->get_client_entity_from_handle(player->get_active_weapon())))
		{
	...
	bool __fastcall perform_prediction(prediction_t* prediction, uint32_t edx, int32_t slot, cs_player_t* player, bool recv_update, int32_t ack, int32_t out)
	{
