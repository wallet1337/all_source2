...
namespace hooks::leaf_system
{
	void __stdcall calc_renderable_world_space_aabb_bloated(renderable_info* info, vec3* mins, vec3* maxs)
	{
		const auto ent = info && info->renderable && info->renderable->get_client_unknown() ?
			reinterpret_cast<cs_player_t*>(info->renderable->get_client_unknown()->get_base_entity()) : nullptr;
		if (ent && (ent->is_player() || ent->get_class_id() == class_id::csragdoll) && game->local_player && game->local_player->is_valid())
		{
	...
	void __stdcall add_renderables_to_render_list(uintptr_t info, int32_t count, renderable_info** renderables, uintptr_t rl_info, int32_t dcount, uintptr_t dinfo)
	{
		for (auto i = 0; i < count; i++)
		{
			...
			if (el->model_type == 1)
			{
				const auto ent = el && el->renderable && el->renderable->get_client_unknown() ?
					reinterpret_cast<cs_player_t*>(el->renderable->get_client_unknown()->get_base_entity()) : nullptr;
	...
	int32_t __stdcall extract_occluded_renderables(int32_t view_id, int32_t count, renderable_info** renderables, uintptr_t rl_info)
	{
		for (auto i = 0; i < count; i++)
		{
			auto el = renderables[i];
