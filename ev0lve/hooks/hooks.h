...
namespace hooks
{
	namespace hl_client
	{
		...
		void __fastcall create_move(sdk::hl_client_t* client, uint32_t edx, int32_t sequence_number, float input_sample_frametime, bool active);
	...
	{
		void __fastcall run_command(sdk::prediction_t* prediction, uint32_t edx, sdk::cs_player_t* player, sdk::user_cmd* cmd, uintptr_t helper);
		bool __fastcall perform_prediction(sdk::prediction_t* prediction, uint32_t edx, int32_t slot, sdk::cs_player_t* player, bool recv_update, int32_t ack, int32_t out);
	}
	...
	{
		void __fastcall packet_start(sdk::client_state_t* state, uint32_t edx, int32_t incoming_sequence, int32_t outgoing_acknowledged);
	...
	namespace leaf_system
	{
		...
		void __stdcall add_renderables_to_render_list(uintptr_t info, int32_t count, sdk::renderable_info** renderables, uintptr_t rl_info, int32_t dcount, uintptr_t dinfo);
		int32_t __stdcall extract_occluded_renderables(int32_t view_id, int32_t count, sdk::renderable_info** renderables, uintptr_t rl_info);
	}

	namespace model_render
	{
		void __fastcall draw_model_execute(uintptr_t render, uint32_t edx, sdk::mat_render_context* context, uintptr_t state, sdk::model_render_info_t* info, sdk::mat3x4* bone);
	}
	...
	namespace entity
	{
		...
		void __fastcall maintain_sequence_transitions(sdk::entity_t* ent, uintptr_t edx, uintptr_t bone_setup, sdk::vec3* pos, sdk::quaternion* q);
	...
	namespace cs_player
	{
		...
		void __fastcall calc_view(sdk::cs_player_t* player, uint32_t edx, sdk::vec3* eye, sdk::angle* ang, float* znear, float* zfar, float* fov);
		void __fastcall fire_event(sdk::cs_player_t* player, uint32_t edx, sdk::vec3* origin, sdk::angle* angles, int event, const char* options);
		void __fastcall obb_change_callback(sdk::cs_player_t* player, uint32_t edx, sdk::vec3* old_mins, sdk::vec3* mins, sdk::vec3* old_maxs, sdk::vec3* maxs);
	...
	namespace miscellaneous
	{
		...
		void __fastcall ent_info_list_link_before(sdk::ent_info_list_t* list, uint32_t edx, sdk::ent_info_t* after, sdk::ent_info_t* element);
		...
		void __fastcall draw_mesh(uintptr_t dx8_api, uint32_t edx, uint32_t mesh, uint32_t count, sdk::mesh_instance_data* instances, uint32_t compression, uint32_t compiled_state, uint32_t info);
		void __fastcall draw_3d_debug_overlays(uintptr_t view, uint32_t edx);
		void __fastcall render_glow_boxes(sdk::glow_manager_t* glow, uintptr_t edx, int32_t pass, sdk::mat_render_context** ctx);
		void __fastcall spawn_smoke_effect(uintptr_t smoke, uint32_t edx);
		void __fastcall render_iron_sight_scope_effect(uintptr_t iron_sight, uint32_t edx, int32_t x, int32_t y, int32_t w, int32_t h, uintptr_t view_setup);
