...
namespace sdk
{
	...
	{		
	public:
		inline void add_glow_box(const vec3& origin, const angle& orient, const vec3& mins, const vec3& maxs, const color& clr, float time = 0.f)
		{
			glow_box_definition def { origin, orient, mins, maxs, time == 0.f ? 0.f : game->globals->curtime - .25f,
				time == 0.f ? 2.f * game->globals->curtime : game->globals->curtime + time , clr.encoded() };
		...
		inline void add_glow(client_entity* const ent, const evo::ren::color& clr)
		{
			for (auto i = 0; i < glow_object_definitions.count(); ++i)
			{
				...
				if (def.ent == ent)
				{
					...
					def.glow_color = { clr.r() / 255.f, clr.g() / 255.f, clr.b() / 255.f };
					def.glow_alpha = { clr.a() / 255.f * .8f };
					break;
				}
			}
		}
