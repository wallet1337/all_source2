...
namespace sdk
{
	...
	{
		typedescription_t(int32_t type, char* name, int32_t offset, int32_t size, float tolerance = 0.f, bool no_error_check = false)
			: field_type(type), field_name(name), offset(offset), size(1), flags(0x100 | (no_error_check ? 0x400 : 0)), external_name(name),
			save_ops(0), input_fn(0), td(nullptr), field_size(size), override_field(nullptr),
