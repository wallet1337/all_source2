...
namespace sdk
{
	...
	public:
		VIRTUAL(6, list_leaves_in_box(sdk::vec3* mins, sdk::vec3* maxs, uint16_t* list, int32_t list_size),
			int32_t(__thiscall*)(void*, sdk::vec3*, sdk::vec3*, uint16_t*, int32_t))(mins, maxs, list, list_size)
	};
	...
	public:
		...
		VIRTUAL(8, get_player_info(const uint32_t index, player_info* info), bool(__thiscall*)(void*, uint32_t, player_info*))(index, info)
		...
		VIRTUAL(114, clientcmd_unrestricted(const char* str, const char* flag), void(__thiscall*)(void*, const char*, const char*))(str, flag)

		inline vec2 get_screen_size() { int32_t x, y; get_screen_size(x, y); return { static_cast<float>(x), static_cast<float>(y) }; }
		inline player_info get_player_info(const uint32_t index) { player_info info; get_player_info(index, &info); return info; }
		inline angle get_view_angles() { angle ang;	get_view_angles(ang); return ang; }
	};
}
