...
{
public:
	CUtlMemory( int nGrowSize = 0, int nInitSize = 0 ) : m_pMemory(0),
		m_nAllocationCount( nInitSize ), m_nGrowSize( nGrowSize )
	{
		if (m_nAllocationCount)
			m_pMemory = (T*) game->mem_alloc->alloc( m_nAllocationCount * sizeof(T) );
	}
	~CUtlMemory() { Purge(); }
	...
	void Grow(int num)
	{
		...
		if (m_pMemory)
			m_pMemory = (T*)game->mem_alloc->realloc( m_pMemory, m_nAllocationCount * sizeof(T) );
		else
			m_pMemory = (T*)game->mem_alloc->alloc( m_nAllocationCount * sizeof(T) );
	}
//protected:
...
public:
	...
	~cutlvector() { remove_all(); m_Memory.Purge(); }
