...
namespace sdk
{
	...
	{
	public:
		VIRTUAL(0, create_renderable_handle(void* obj), void(__thiscall*)(void*, uintptr_t, int, char, int, int))(reinterpret_cast<uintptr_t>(obj) + 0x4, 0, 0, 0xFFFFFFFF, 0xFFFFFFFF)
		VIRTUAL(1, remove_renderable(base_handle handle), void(__thiscall*)(void*, base_handle))(handle)
		VIRTUAL(15, renderable_changed(const client_render_handle handle), void(__thiscall*)(void*, client_render_handle))(handle)
	};
	...
	{
	private:
		FUNC(game->client.at(functions::modifier_table::find), find(uint16_t* symbol, char* lookup), uint16_t*(__thiscall*)(void*, uint16_t*, char*))(symbol, lookup)
		FUNC(game->client.at(functions::modifier_table::add_string), add_string(uint16_t* symbol, char* lookup), uint16_t*(__thiscall*)(void*, uint16_t*, char*))(symbol, lookup);
	...
	struct str_table
	{
		VIRTUAL(8, add_string(bool is_server, const char* val), int32_t(__thiscall*)(void*, bool, const char*, int32_t, void*))(is_server, val, -1, nullptr)
	};
	...
	{
		return reinterpret_cast<uintptr_t(__thiscall*)(uintptr_t, const char*)>(game->client.at(functions::find_hud_element))(game->client.at(globals::hud_instance), name);
	}
	...
	inline void draw_sphere(const vec3& pos, float rad, const color& clr, int32_t theta_phi = 12)
	{
		...
		((void(__cdecl*)(const void*, float, int32_t, int32_t, uint32_t, uintptr_t, bool))game->engine.at(functions::draw_sphere::fn))
			(&pos, rad, theta_phi, theta_phi, clr.encoded(), *(uintptr_t*)game->client.at(functions::draw_sphere::material), true);
	}
