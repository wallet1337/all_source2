...
namespace sdk
{
	...
	public:
		...
		VIRTUAL(14, set_string(const char* v), void(__thiscall*)(convar*, const char*))(v)
		VIRTUAL(15, set_float(const float v), void(__thiscall*)(convar*, float))(v)
		VIRTUAL(16, set_int(const int32_t v), void(__thiscall*)(convar*, int32_t))(v)
		...
		inline void enforce_sent_value(bool& current, bool sent)
		{
			...
			char val[] = { sent ? '1' : '0', '\0' };
			const auto bak = string;
			string = val;
			hooks::miscellaneous::convar_network_change_callback(reinterpret_cast<convar*>(uintptr_t(this) + 0x18), nullptr, NAN);
			string = bak;
			current = sent;
		}
...
#define GET_CONVAR_INT(name) []() -> int32_t { static const auto var = util::encrypted_ptr<sdk::convar>(game->cvar->find_var(XOR_STR(name))); return var->get_int(); } ()
#define GET_CONVAR_FLOAT(name) []() -> float { static const auto var = util::encrypted_ptr<sdk::convar>(game->cvar->find_var(XOR_STR(name))); return var->get_float(); } ()
#define GET_CONVAR_STR(name) []() -> char* { static const auto var = util::encrypted_ptr<sdk::convar>(game->cvar->find_var(XOR_STR(name))); return var->string; } ()
#define SET_CONVAR_INT(name, val) [&]() { static const auto var = util::encrypted_ptr<sdk::convar>(game->cvar->find_var(XOR_STR(name))); return var->set_int(val); } ()
#define SET_CONVAR_FLOAT(name, val) [&]() { static const auto var = util::encrypted_ptr<sdk::convar>(game->cvar->find_var(XOR_STR(name))); return var->set_float(val); } ()
#define SET_CONVAR_STR(name, val) [&]() { static const auto var = util::encrypted_ptr<sdk::convar>(game->cvar->find_var(XOR_STR(name))); return var->set_string(val); } ()
#define DISPATCH_COMMAND(name) [&]() { static const auto var = util::encrypted_ptr<sdk::concommand>(game->cvar->find_command(XOR_STR(name))); var->dispatch(); } ()
