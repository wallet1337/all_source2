...
namespace sdk
{
	...
	__forceinline vertex_t rotate_vertex(const vec2& o, const vertex_t& v, const float angle)
	{
		...
		{
			{
				o.x + (v.pos.x - o.x) * c - (v.pos.y - o.y) * s,
				o.y + (v.pos.x - o.x) * s + (v.pos.y - o.y) * c
			}
		};
	}
	...
	{
		const auto sp = sin(DEG2RAD(ang.x)), cp = cos(DEG2RAD(ang.x)),
			sy = sin(DEG2RAD(ang.y)), cy = cos(DEG2RAD(ang.y));
	...
	__forceinline vec3 matrix_position(const mat3x4& matrix)
	{
	...
	__forceinline angle matrix_angles(const mat3x4& matrix)
	{
		...
		if (xy_dist > 0.001f)
			return angle(RAD2DEG(atan2f(-forward[2], xy_dist)),
	...
	__forceinline vec3 matrix_get_origin(const mat3x4& src)
	{
	...
	{
		return vec3(
			start.x + std::cos(DEG2RAD(rotation)) * length_remaining,
			start.y + std::sin(DEG2RAD(rotation)) * length_remaining,
			start.z);
	}
