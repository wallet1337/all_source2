...
static const __declspec(align(16)) int32_t component_mask[4][4]{ { (int32_t)0xFFFFFFFF, 0, 0, 0 }, { 0, (int32_t)0xFFFFFFFF, 0, 0 }, { 0, 0, (int32_t)0xFFFFFFFF, 0 }, { 0, 0, 0, (int32_t)0xFFFFFFFF } };
...
__forceinline float& sub_float(m128& a, int idx)
{
	return a.f[idx];
}

__forceinline uint32_t& sub_uint(m128& a, int idx)
{
	return a.u[idx];
}
...
__forceinline m128 add_simd(m128& a, m128& b)
{
	BINOP(+);
}

__forceinline m128 sub_simd(m128& a, m128& b)
{
	BINOP(-);
};

__forceinline m128 mul_simd(m128& a, m128& b)
{
	BINOP(*);
}

__forceinline m128 div_simd(m128& a, m128& b)
{
	BINOP(/ );
}

__forceinline m128 load_simd(const void* pSIMD)
{
	return *(reinterpret_cast<const m128*>(pSIMD));
}

__forceinline void store_simd(float* pSIMD, const m128& a)
{
	*(reinterpret_cast<m128*>(pSIMD)) = a;
}
