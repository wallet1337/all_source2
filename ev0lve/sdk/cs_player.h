...
namespace sdk
{
	...
	inline static constexpr auto use_no_ik_context = 2;
	...
	public:
		...
		{
			return sequence != other.sequence || weight != other.weight ||
				weight_delta_rate != other.weight_delta_rate ||
				playback_rate != other.playback_rate || cycle != other.cycle ||
				order != other.order;
		}
		
		inline void increment_layer_cycle(const float delta, const bool loop)
		{
			if (fabs(playback_rate) <= 0.f)
				return;
		...
		inline bool has_sequence_completed(const float delta = 0.f) const
		{
			return cycle + playback_rate * delta >= 1.f;
		}
		
		inline void reset()
		{
			sequence = order = 0;
			prev_cycle = weight = weight_delta_rate
				= playback_rate = cycle = 0.f;
		}
	};
	...
	struct alignas(16) bone_snapshot
	{
		entity_t* player;
		float weight;
		mat3x4 cache[max_bones];
		const char*	weight_list_name;
		float weight_list[max_bones];
	...
	struct anim_state
	{
		...
		FUNC(game->client.at(functions::anim_state::get_active_weapon_prefix), get_active_weapon_prefix() const, const char*(__thiscall*)(const void*))()
		FUNC(game->client.at(functions::anim_state::update_layer_order_preset), update_layer_order_preset(int32_t index, int32_t sequence), void(__thiscall*)(void*, int32_t, int32_t))(index, sequence)

		inline void update(float yaw, float pitch)
		{
			auto& enable_invalidate_bone_cache = *(bool*) game->client.at(offsets::base_animating::enable_invalidate_bone_cache);
			const auto bak = enable_invalidate_bone_cache;
			((void(__vectorcall*)(void*, uint32_t, float, float, float, bool)) game->client.at(functions::anim_state::update))(this, 0, 0.f, yaw, pitch, false);
	...
	typedef std::array<uint8_t, 0x210> local_data_t;

	class cs_player_t : public entity_t
	{
		FUNC(game->client.at(offsets::csplayer::fn_is_other_enemy), is_other_enemy(cs_player_t* other), bool(__thiscall*)(cs_player_t*, cs_player_t*))(other)
	...
	public:
		...
		VAR(offsets::base_combat_character, next_attack, float)
		...
		VAR(offsets::base_player, duck_amount, float)
		VAR(offsets::base_player, duck_speed, float)
		VAR(offsets::base_player, surface_friction, float)
		VAR(offsets::base_player, ground_entity, base_handle)
		...
		VAR(offsets::csplayer, is_ragdoll, bool)
		...
		VAR(offsets::csplayer, cycle, float)
		VAR(offsets::csplayer, assassination_target_addon, base_handle)

		VAR(offsets::csragdoll, player, base_handle)
		VAR(offsets::cslocal_player_exclusive, velocity_modifier, float)
		...
		MEMBER(offsets::base_player::health, player_health, int32_t)
		MEMBER(offsets::csplayer::armor_value, player_armor, int32_t)
		MEMBER(offsets::csplayer::has_defuser, player_defuser, bool)
		MEMBER(offsets::csplayer::flash_duration - 0xC, flash_alpha, float)
		MEMBER(offsets::local_player_exclusive::view_offset_0, view_offset, vec3)
		MEMBER(offsets::local_player_exclusive::friction, friction, float)
		MEMBER(offsets::local_player_exclusive::local, local, local_data_t)

		VIRTUAL(170, eye_angles(), sdk::angle*(__thiscall*)(void*))()
		VIRTUAL(206, standard_blending_rules(sdk::cstudiohdr* hdr, sdk::vec3* pos, sdk::quaternion_aligned* q, float time, int32_t mask), void(__thiscall*)
			(void*, sdk::cstudiohdr*, sdk::vec3*, sdk::quaternion_aligned*, float, int32_t))(hdr, pos, q, time, mask)
		VIRTUAL(223, get_layer_sequence_cycle_rate(animation_layer* layer, int32_t sequence), float(__thiscall*)(void*, animation_layer*, int32_t))(layer, sequence)
		VIRTUAL(263, owns_this_type(const char* type), sdk::weapon_t*(__thiscall*)(void*, const char*, int32_t))(type, 0)
		VIRTUAL(264, get_slot(int32_t slot), sdk::weapon_t*(__thiscall*)(void*, int32_t))(slot)

		FUNC(game->client.at(offsets::csplayer::fn_post_build_transformations), post_build_transformations(sdk::mat3x4* out, uint32_t mask), void(__thiscall*)(cs_player_t*, sdk::mat3x4*, uint32_t))(out, mask)
		FUNC(game->client.at(offsets::csplayer::fn_update_addon_models), update_addon_models(bool force), int32_t(__thiscall*)(cs_player_t*, bool))(force)
		...
		inline bool is_enemy(cs_player_t* other = nullptr)
		{
			if (!other)
			{
				other = reinterpret_cast<cs_player_t*>(game->client_entity_list->get_client_entity(game->engine_client->get_local_player()));

				if (other && !other->is_alive())
					other = reinterpret_cast<cs_player_t*>(game->client_entity_list->get_client_entity_from_handle(other->get_observer_target()));
			}
		...
		{
			return is_alive() && !get_is_ragdoll() &&
				(game->local_player == this || !is_dormant() || not_dormant)
				&& index() >= 1 && index() <= 65;
		}
		...
		inline bool is_fake_player()
		{
			return game->engine_client->get_player_info(index()).fakeplayer;
		}
		...
		inline vec3 get_eye_position()
		{
			...
			else
			{
				...
				if (is_on_ground())
					res -= (game->cs_game_movement->get_view_offset(false) - game->cs_game_movement->get_view_offset(true)) * get_duck_amount();
			}
		...
		inline angle get_punch_angle_scaled()
		{
			return get_aim_punch_angle() * GET_CONVAR_FLOAT("weapon_recoil_scale");
		}
		...
		inline void try_initiate_animation(size_t layer, int32_t activity, std::vector<uint16_t>& modifiers, uint32_t seed)
		{
			...
			if (!hdr || !hdr->sequences_available() || hdr->get_numseq() == 1 || modifiers.size() == 0 || !state)
				return;
			...
			if (uintptr_t(mapping) != empty_mapping && (mapping->studio_hdr != hdr->hdr || mapping->v_model != hdr->v_model))
