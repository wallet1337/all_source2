...
namespace util
{
	...
	{
	public:
		explicit hook(const encrypted_ptr<T> target, const encrypted_ptr<T> endpoint) : hook_interface(), target(target), original_bytes(), is_attached(false)
		{
			const auto length_disasm = [](uint8_t* op) -> uint32_t
			{
				uint32_t size, code;
				if (*op == 0x8A) return 2;
				return ((bool(__cdecl*)(uint8_t*, uint32_t*, uint32_t*)) game->gameoverlayrenderer.at(sdk::functions::gameoverlayrenderer::length_disasm))(op, &size, &code) ? size : 0xFFFF;
			};
			...
			*reinterpret_cast<uint32_t*>(&jmp_instruction[1]) = reinterpret_cast<uint32_t>(ptr) - reinterpret_cast<uint32_t>(trampoline.data()) - jmp_instruction.size();
		...
		void attach() override
		{
			...
			const auto ptr = reinterpret_cast<uint8_t*>(target());
			std::copy(&ptr[0], &ptr[5], original_bytes.data());

			mem_guard mem(PAGE_EXECUTE_READWRITE, target(), original_bytes.size());
			ptr[0] = 0xE9; // JMP rel32
			*reinterpret_cast<uint32_t*>(&ptr[1]) = reinterpret_cast<uint32_t>(endpoint()) - reinterpret_cast<uint32_t>(&ptr[5]);
			FlushInstructionCache(CURRENT_PROCESS, ptr, original_bytes.size());
		...
		{
			return reinterpret_cast<T*>(const_cast<uint8_t*>(trampoline.data()));
		}
...
#define MAKE_HOOK(fn, name, pat) std::make_unique<util::hook<decltype(fn)>>(PATTERN(decltype(fn), name, pat), util::encrypted_ptr(fn))
#define MAKE_HOOK_OFFSET(fn, name, pat, off) std::make_unique<util::hook<decltype(fn)>>(PATTERN_OFFSET(decltype(fn), name, pat, off), util::encrypted_ptr(fn))
#define MAKE_HOOK_REL(fn, name, pat) std::make_unique<util::hook<decltype(fn)>>(PATTERN_REL(decltype(fn), name, pat), util::encrypted_ptr(fn))
#define MAKE_HOOK_PRIMITIVE(fn, to) std::make_unique<util::hook<decltype(fn)>>(util::encrypted_ptr<decltype(fn)>((decltype(&fn))to), util::encrypted_ptr(fn))
