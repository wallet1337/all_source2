...
{
	buf.push_aa(true);
	buf.add_line(ren::vec2(from.x, from.y).floor(), ren::vec2(to.x, to.y).floor(), ren::color(cl.red(), cl.green(), cl.blue(), cl.alpha()), thickness);
	buf.pop_aa();
}

void dx_adapter::rect(const vec2& from, const vec2& to, const color& cl) const
{
	buf.add_rect(ren::rect({ from.x, from.y }, { to.x, to.y }).floor(), ren::color(cl.red(), cl.green(), cl.blue(), cl.alpha()));
}

void dx_adapter::rect_filled(const vec2& from, const vec2& to, const color& cl) const
{
	buf.add_rect_filled(ren::rect({ from.x, from.y }, { to.x, to.y }).floor(), ren::color(cl.red(), cl.green(), cl.blue(), cl.alpha()));
}

void dx_adapter::rect_filled_fade(const vec2 &from, const vec2 &to, const color& cl0, const color& cl1, const bool horizontal) const
{
	const auto c0 = ren::color(cl0.red(), cl0.green(), cl0.blue(), cl0.alpha());
	const auto c1 = ren::color(cl1.red(), cl1.green(), cl1.blue(), cl1.alpha());
	buf.add_rect_filled_multicolor(ren::rect({ from.x, from.y }, { to.x, to.y }).floor(), { c0, horizontal ? c0 : c1, c1, horizontal ? c1 : c0 });
}

void dx_adapter::circle(const vec2& center, int32_t radius, const color& cl, float progress, bool filled) const
{
	const auto segments = static_cast<int32_t>(radius * 1.5f);
	buf.push_aa(true);
	if (filled)
		buf.add_circle_filled(ren::vec2(center.x, center.y).floor(), radius, ren::color(cl.red(), cl.green(), cl.blue(), cl.alpha()), segments, progress);
	else
		buf.add_circle(ren::vec2(center.x, center.y).floor(), radius, ren::color(cl.red(), cl.green(), cl.blue(), cl.alpha()), 1.f, segments, progress);
	buf.pop_aa();
}
...
void dx_adapter::circle_filled_fade(const vec2& center, int32_t radius, const color& cl, float progress) const
{
	const auto segments = static_cast<int32_t>(radius * 1.5f);
	buf.push_aa(true);
	buf.add_circle_filled_multicolor(ren::vec2(center.x, center.y).floor(), radius,
			{ ren::color(cl.red(), cl.green(), cl.blue(),static_cast<int32_t>(.8f * cl.alpha())),
					ren::color(cl.red(), cl.green(), cl.blue(), static_cast<int32_t>(.2f * cl.alpha() * progress)) }, segments);
	buf.pop_aa();
}
...
void dx_adapter::text(const vec2 &pos, const color &cl, const wchar_t *const text, const uint32_t font, const gui_legacy::horizontal_alignment horizontal, const gui_legacy::vertical_alignment vertical) const
{
	std::string utf8_str{};
	...
	switch (horizontal)
	{
		...
		case gui_legacy::horizontal_alignment::right:
			ah = ren::layer::align_right;
			break;
		case gui_legacy::horizontal_alignment::center:
			ah = ren::layer::align_center;
			break;
	}
	...
	switch (vertical)
	{
		...
		case gui_legacy::vertical_alignment::bottom:
			av = ren::layer::align_bottom;
			break;
		case gui_legacy::vertical_alignment::center:
			av = ren::layer::align_center;
			break;
	}
	
	buf.add_text(ren::draw.get_font(font), ren::vec2(pos.x, pos.y).floor(), utf8_str, ren::color(cl.red(), cl.green(), cl.blue(), cl.alpha()), ah, av);
}
...
gui_legacy::vec2 dx_adapter::get_text_size(const wchar_t *const text, const uint32_t font) const
{
	...
	const auto sz = ren::text_size(ren::draw.get_font(font), utf8_str);
	return { sz.x, sz.y };
}

float dx_adapter::get_alpha_modifier() const
{
	return buf.get_alpha().value_or(1.f);
}

void dx_adapter::set_alpha_modifier(const float modifier) const
{
	buf.pop_alpha().push_alpha(modifier);
}

std::shared_ptr<evo::ren::texture> dx_adapter::load_svg_to_texture(const uint8_t *buffer, size_t size, uint32_t w, uint32_t h) const
{
...
void dx_adapter::weapon(const vec2 &pos, const float height, const color &clr, const char *name) const
{
	...
	buf.push_texture(wpn->texture);
	buf.add_rect_filled(ren::rect({ from.x + 1, from.y + 1 }, { to.x + 1, to.y + 1 }).floor(), ren::color(0, 0, 0, static_cast<uint8_t>(.6f * static_cast<float>(clr.alpha()))));
	buf.add_rect_filled(ren::rect({ from.x , from.y }, { to.x, to.y }).floor(), ren::color(clr.red(), clr.green(), clr.blue(), clr.alpha()));
	buf.pop_texture();
}
