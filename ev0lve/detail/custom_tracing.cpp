...
namespace detail
{
	...
	custom_tracing::wall_pen custom_tracing::wall_penetration(vec3 src, vec3 end, const std::shared_ptr<lag_record> target, bool scan_secure,
			std::optional<resolver_direction> override_direction, cs_player_t* const override_player, bool no_opt, cs_weapon_data* override_data)
	{
		const auto player = override_player ? override_player : game->local_player;
		const auto wpn = override_data ? nullptr : reinterpret_cast<weapon_t*>(game->client_entity_list
			->get_client_entity_from_handle(player->get_active_weapon()));
		...
		if (target)
		{
			...
			result = fire_bullet(wpn->get_cs_weapon_data(), src, end, &filter, target, scan_secure, override_direction,
					override_player ? false : wpn->get_item_definition_index() == item_definition_index::weapon_taser, no_opt);
		}
		else
		{
			if (override_data)
			{
				...
				result = fire_bullet(override_data, src, end, &filter, target, scan_secure, override_direction,false, no_opt);
			}
			else
			{
	...
	void custom_tracing::check_wallbang()
	{
		...
		const auto wpn = reinterpret_cast<weapon_t * const>(game->client_entity_list->get_client_entity_from_handle(
		...
		wallbang = handle_bullet_penetration(wpn_data, tr, eye_pos, direction, penetration_count, damage, wpn_data->flpenetration);
	}

	custom_tracing::wall_pen custom_tracing::fire_bullet(cs_weapon_data* const data, vec3 src, vec3 pos, trace_filter* const filter,
		...
		while (damage > 0.f)
		{
			...
			if (target && !pen.did_hit)
			{
				...
				if (behind || enter_trace.fraction == 1.f)
				{
					...
					pen.did_hit = true;
					pen.secure_point = scan_secure;
					pen.very_secure = false;
					pen.damage = 0.f;

					const auto test = [&](const mat3x4* mat, bool real = false)
					{
						ray r{};
						r.init(src, src + direction * (length_remaining + ray_extension));
						game_trace tr{};
						tr.contents = enter_trace.contents;
						tr.startpos = r.start + r.start_offset;
						tr.endpos = tr.startpos + r.delta;
						for (auto i = 0; i < max_bones; i++)
							mat_[i] = const_cast<mat3x4*>(&mat[i]);
						const auto ret = trace_to_studio_csgo_hitgroups_priority(target->player, mask_shot_player, &target->origin, &tr, &r, mat_);
						auto smallest_fraction = (src - tr.endpos).length() / length_remaining;
						if (const auto range = distance_to_ray( target->origin + vec3(0.f, 0.f, .5f * target->player->get_collideable()->obb_maxs().z), r.start, r.start + r.delta);
							range >= 0.0f && range <= 60.f && tr.fraction < smallest_fraction)
							smallest_fraction = tr.fraction;
						const auto final_length = length + smallest_fraction * (length_remaining + ray_extension);

						if (!ret || final_length >= max_length)
						{
							pen.secure_point = false;
							pen.min_damage = 0.f;
							return ret;
						}

						const auto final_damage = damage * std::powf(data->flrangemodifier, final_length * .002f);
						const auto new_damage = scale_damage(target->player, final_damage, data->flarmorratio, is_zeus ? hitgroup_generic : tr.hitgroup);

						if (!pen.secure_point || new_damage < pen.min_damage)
							pen.min_damage = new_damage;

						if (!scan_secure)
							pen.impacts[pen.impact_count++] = tr.endpos;

						if (real)
						{
							pen.end = tr.endpos;
							pen.hitbox = cs_player_t::hitbox(tr.hitbox);
							pen.hitgroup = tr.hitgroup;
							pen.damage = new_damage;
							pen.potential_damage = scale_damage(target->player, data->idamage * std::powf(data->flrangemodifier,
									(start - pen.end).length() * .002f), data->flarmorratio, is_zeus ? hitgroup_generic : tr.hitgroup);
						}
					...
					if (scan_secure)
					{
						pen.min_damage = INT_MAX;

						if (test(target->res_mat[target->res_direction], true))
							if ((target->res_direction == resolver_max || test(target->res_mat[resolver_max]))
									&& (target->res_direction == resolver_min || test(target->res_mat[resolver_min])))
							{
								pen.very_secure = true;
								const auto min_damage = pen.min_damage;

								// regular case.
								if (!((target->res_direction == resolver_max_max || test(target->res_mat[resolver_max_max]))
										&& (target->res_direction == resolver_min_min || test(target->res_mat[resolver_min_min]))
										&& (target->res_direction == resolver_min_extra || test(target->res_mat[resolver_min_extra]))
										&& (target->res_direction == resolver_max_extra || test(target->res_mat[resolver_max_extra])))
										// extrapolation case.
										|| (target->extrapolated && target->previous && !(test(target->previous->res_mat[resolver_max_max])
										&& test(target->previous->res_mat[resolver_min_min]))))
								{
									pen.very_secure = false;
									pen.secure_point = true;
									pen.min_damage = min_damage;
								}
							}
					}
					else
						test(override_direction.has_value() ? target->res_mat[override_direction.value()] : target->mat, true);

					if (target->player->is_fake_player())
						pen.secure_point = pen.very_secure = true;

					if (pen.damage <= 1.f)
						pen.secure_point = pen.very_secure = false;
			...
			if ((length > 3000.f && data->flpenetration > 0.f) || (!enter_surface || enter_surface->game.penetration_mod < .1f))
				penetrate_count = 0;
	...
	bool custom_tracing::handle_bullet_penetration(const cs_weapon_data* const weapon_data, game_trace& enter_trace, vec3& eye_position, vec3 direction, int& penetrate_count, float& current_damage, float penetration_power) const
	{
		...
		if ((penetrate_count == 0 && is_grate && !is_no_draw && enter_material != char_tex_grate && enter_material != char_tex_glass)
			|| (weapon_data->flpenetration <= 0.f || penetrate_count <= 0)
		...
		else if (enter_material == char_tex_flesh && enemy && !enemy->is_enemy() && GET_CONVAR_FLOAT("ff_damage_reduction_bullets") == 0.f)
		{
	...
	bool custom_tracing::trace_to_exit(game_trace& enter_trace, game_trace& exit_trace, vec3 start_position, vec3 direction) const
	{
		...
		while (current_distance <= max_distance)
		{
			...
			if (!first_contents)
				first_contents = game->engine_trace->get_point_contents_world_only(start, mask_shot_player);
	...
	float custom_tracing::scale_damage(cs_player_t* const target, float damage, float weapon_armor_ratio, int hitgroup) const
	{
	...
	bool __declspec(noinline) custom_tracing::trace_to_studio_csgo_hitgroups_priority(cs_player_t* player, uint32_t contents_mask, vec3* origin, game_trace* tr, ray* r, mat3x4** mat) const
	{
