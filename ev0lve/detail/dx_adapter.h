...
namespace detail
{
	...
	public:
		inline static constexpr uint8_t esp_layer = 50;
		...
		void rect_filled_fade(const vec2 &from, const vec2 &to, const color &cl0, const color &cl1, const bool horizontal = false) const final;
		void circle(const vec2 &center, int32_t radius, const color &cl, float progress = 1.f, bool filled = true) const final;
		...
		void text(const vec2 &pos, const color &cl, const wchar_t *const text, const uint32_t font = default_font, const gui_legacy::horizontal_alignment horizontal = gui_legacy::horizontal_alignment::left, const gui_legacy::vertical_alignment vertical = gui_legacy::vertical_alignment::top) const final;
		...
		std::shared_ptr<evo::ren::texture> load_svg_to_texture(const uint8_t *buffer, size_t size, uint32_t w, uint32_t h) const;
