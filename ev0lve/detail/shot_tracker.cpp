...
namespace detail
{
	...
	void shot_tracker::register_shot(shot&& s)
	{
	...
	void shot_tracker::on_bullet_impact(game_event* event)
	{
		...
		last_confirmed->server_info.impacts.emplace_back(event->get_float(XOR_STR("x")),
	...
	{
		const auto player = s.record != nullptr ? reinterpret_cast<cs_player_t*>(game->client_entity_list->get_client_entity(s.record->index)) : nullptr;
		...
		if (!s.manual && player)
		{
			...
			{
				org_pen = trace.wall_penetration(s.start, end, s.record, false, s.target_direction);
				s.record = std::make_shared<lag_record>(entry.records.front());
				s.record->perform_matrix_setup();
			}

			pen = trace.wall_penetration(s.start, end, s.record, false, s.target_direction);

			if ((org_start - s.start).length2d() > .01f)
				hit_start = trace.wall_penetration(org_start, end, s.record, false, s.target_direction).did_hit;
...
#ifdef CSGO_LUA
		lua::api.callback(FNV1A("on_shot_fired"), [&](lua::state& state) {
			XOR_STR_STACK(miss_reason_dec, miss_reason);

			state.create_table();
			state.set_field(XOR_STR("manual"), s.manual);
			state.set_field(XOR_STR("secure_point"), pen.secure_point);
		...
		if (cfg.local_visuals.impacts.get())
			for (const auto& impact : s.server_info.impacts)
				game->debug_overlay->box_overlay_2(impact, { -1.25f, -1.25f, -1.25f }, { 1.25f, 1.25f, 1.25f }, sdk::angle(),
					cfg.local_visuals.server_impacts.get(), sdk::color(250, 250, 250,
						static_cast<uint8_t>(.25f * cfg.local_visuals.server_impacts.get().a())), 4.f);

		if ((cfg.legit.enable.get() && !cfg.rage.enable.get()) || !player || s.manual || s.record->dormant || !game->local_player->is_alive())
			return;
