...
int api_def::cvar::index(lua_State *l)
{
	...
	if (cv->flags & FCVAR_CHEAT || cv->flags & FCVAR_DEVELOPMENTONLY)
	{
		...
		{
			s.error(XOR_STR("cannot access cheat or dev only convars with unsafe mode disabled"));
			return 0;
		}
	}

	const auto is_blocked = std::find(helpers::blocked_vars.begin(), helpers::blocked_vars.end(), util::fnv1a(cv->name)) != helpers::blocked_vars.end();
	const auto is_risky = std::find(helpers::risky_vars.begin(), helpers::risky_vars.end(), util::fnv1a(cv->name)) != helpers::risky_vars.end();
	if (is_blocked || is_risky)
	{
		if (!cfg.lua.allow_insecure.get())
		{
			s.error(XOR_STR("convar is blocked from access with unsafe mode disabled"));
			return 0;
		}
	}
