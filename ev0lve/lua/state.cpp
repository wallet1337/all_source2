...
{
	print_to_console(XOR_STR("--- STACK TRACE DUMP ---\n"));
	for (auto i = lua_gettop(l); i >= 1; --i)
	{
		print_to_console(XOR_STR("#%i:\ttypename %s => "), i, lua_typename(l, lua_type(l, i)));

		switch (lua_type(l, i))
		{
			case LUA_TNUMBER:
				print_to_console(XOR_STR("%g\n"), lua_tonumber(l, i));
				break;
			case LUA_TSTRING:
				print_to_console(XOR_STR("%s\n"), lua_tostring(l, i));
				break;
			case LUA_TBOOLEAN:
				print_to_console(XOR_STR("%s\n"), lua_toboolean(l, i) ? XOR_STR("true") : XOR_STR("false"));
				break;
			case LUA_TNIL:
				print_to_console(XOR_STR("nil\n"));
				break;
			default:
				print_to_console(XOR_STR("pointer %p\n"), lua_topointer(l, i));
				break;
		}
	}
	
	print_to_console(XOR_STR("--- END STACK TRACE DUMP ---\n"));
}
