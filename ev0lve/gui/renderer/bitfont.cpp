...
bitfont::bitfont(const std::string& nam, float size, uint32_t flg, uint32_t from, uint32_t to)
{
	name = nam;
	height = size;
	flags = flg;
...
void bitfont::create()
{
	...
	if (!device)
		throw std::runtime_error("Couldn't create font device context");

	SetMapMode(device, MM_TEXT);

	win_font = CreateFontA(height, 0, 0, 0, 400, false, false, false,
			ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, NONANTIALIASED_QUALITY,DEFAULT_PITCH | FF_DONTCARE, name.c_str());
	...
	if (!GetTextMetricsA(device, &metric))
		throw std::runtime_error("Couldn't get font metrics");
